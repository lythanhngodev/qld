...
$kq = array(
    ...
    $stcct = $_POST['stcct'];
    $gcct = $_POST['gcct'];
    $sqlk = "SELECT * FROM CTDAOTAO WHERE (MACTDT=:mct)";
    ...
    if (empty($_FILES['file'])){
        $sql = "INSERT INTO CTDAOTAO (IDNDT, IDKHOA, MACTDT, TENCTDT, SOHOCPHAN, SOTINCHI, GHICHU) VALUES (:ndtct, :kct, :mct, :tct, :shpct, :stcct, :gcct)";
        ...
        $sql = "INSERT INTO CTDAOTAO (IDNDT, IDKHOA, MACTDT, TENCTDT, SOHOCPHAN, SOTINCHI, GHICHU, FILES) VALUES (:ndtct, :kct, :mct, :tct, :shpct, :stcct, :gcct, :files)";
        $p_sql = oci_parse($conn, $sql);
        oci_bind_by_name($p_sql, ":ndtct", $ndtct);
        oci_bind_by_name($p_sql, ":kct",$kct);
        oci_bind_by_name($p_sql, ":mct",$mct);
